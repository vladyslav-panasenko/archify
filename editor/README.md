...
- **Copy and duplicate** works for compatible selections across all five diagram types. Ctrl/Cmd+D duplicates; Ctrl/Cmd+C and V copy between documents or editor windows when focus is on the canvas. Pasted items get fresh IDs; internal edges and supported coordinates are remapped. Lane and stage dependencies are checked, and connections to unselected external items are counted and omitted explicitly.
...
- **Checkpoints** stores a configurable one-to-ten named snapshots and 2 MB per document in browser storage. Select records to export or remove, inspect dates and sizes, and export a versioned recovery bundle containing the draft, optional bounded history, and selected checkpoints. Imports validate versions and require an explicit collision choice.
...
Templates: select items in any diagram type and use Templates to save a named, categorized fragment with its internal connections. Import/export individual templates or a versioned library, rename or delete entries, and insert compatible fragments with fresh IDs. Duplicate names in a category require explicit replacement. Templates use browser storage (20 entries / 2 MB); inserts are validated and undoable.
...
Project workspace: launch `npm start -- --directory "D:\Diagrams"` (or use `--file`, not both). Search paths and bounded file content, reopen recent files, and keep several project files in visible tabs with isolated history, selection, draft and recovery state. Create safe nested folders and revision-check a rename or move; existing destinations, traversal and symlinks are rejected. Recovery and view keys follow a moved active file. An opt-in `.archify-editor.json` shares versioned canvas defaults while machine history and diagram content remain separate. Batch validates up to 100 selected files or renders HTML into an existing confined folder with per-file results, cancellation and explicit overwrite.

Compare layout: import a compatible version, then check individual node placements to see orange ghost boxes on the canvas. Accept selected placements creates one undo step. Node identities, endpoints, lane definitions and membership must match. Labels, routes and message timing remain unchanged; sequence participant order is accepted as one group. Locked items are excluded. Cancel removes the preview without changing JSON.
...
Undo recovery: restoring an unsaved draft also restores up to 20 retained past/future states, bounded to 1,048,576 serialized characters of history metadata. Snapshots are validated before restoration. Legacy drafts and invalid history restore the applied document without history; unapplied text and source-revision protections still apply. Saved documents retain history only when the user opts in; retained records are revision-bound, revalidated, bounded to 20 states / 1 MB, and expire after seven days.

Delivery exports: the compiler preview can download self-contained HTML, rasterize its SVG as a bounded 1×–3× PNG with a white background, or open browser printing for PDF with A4/Letter portrait/landscape page rules. Browser font availability affects PNG text. Very wide PDF diagrams are scaled to the page; use HTML when native canvas dimensions must remain exact.
