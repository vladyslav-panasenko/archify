...
test('auto-arrangement previews, cancels and applies one undoable edit',async({page})=>{
 await page.goto('/');await page.locator('.react-flow__node-component').first().click();
 ...
 await page.getByRole('button',{name:'Auto-arrange',exact:true}).click();await page.getByRole('button',{name:'Preview arrangement'}).click();await page.getByRole('button',{name:'Apply arrangement'}).click();
