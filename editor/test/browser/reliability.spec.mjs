...
test('layout comparison previews ghost boxes and accepts selected placement with undo',async({page})=>{
 await page.goto('/');await page.getByRole('button',{name:'JSON',exact:true}).click();const before=await page.getByLabel('Diagram JSON').inputValue(),other=JSON.parse(before);other.components[0].pos=[180,280];other.components[1].pos=[80,90];other.components[0].label='Ignore this label';
 await page.getByRole('button',{name:'Compare layout',exact:true}).click();await page.getByLabel('Compare layout JSON', {exact:true}).setInputFiles({name:'layout.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(other))});await page.getByRole('checkbox',{name:'Users pos',exact:true}).check();await expect(page.locator('.comparison-ghost')).toHaveCount(1);await page.screenshot({path:'test-results/layout-comparison.png'});await page.getByRole('button',{name:'Accept selected placements'}).click();await page.getByRole('button',{name:'JSON',exact:true}).click();const next=JSON.parse(await page.getByLabel('Diagram JSON').inputValue());expect(next.components[0].pos).toEqual([180,280]);expect(next.components[0].label).toBe('Users');expect(next.components[1]).toEqual(JSON.parse(before).components[1]);await page.getByRole('button',{name:'Undo',exact:true}).click();await expect(page.getByLabel('Diagram JSON')).toHaveValue(before);
});
