...
test('Save As creates a workspace copy and requires explicit replacement',async({page})=>{
 const directory=await fs.mkdtemp(path.join(os.tmpdir(),'archify-saveas-ui-'));await fs.writeFile(path.join(directory,'A.json'),JSON.stringify(newDocument('A')));const server=await createEditorServer({directory});await new Promise(r=>server.listen(0,'127.0.0.1',r));
 try{await page.goto(`http://127.0.0.1:${server.address().port}`);await page.getByRole('button',{name:'Save As',exact:true}).click();await page.getByLabel('Project filename').fill('B.json');await page.getByRole('button',{name:'Save as project file',exact:true}).click();await expect(page.getByRole('status')).toContainText('Saved B.json');await expect(page.getByRole('combobox',{name:'Project diagram',exact:true})).toContainText('B.json');expect(JSON.parse(await fs.readFile(path.join(directory,'B.json'),'utf8')).meta.title).toBe('A');await page.getByRole('button',{name:'Save As',exact:true}).click();await page.getByLabel('Project filename').fill('A.json');await page.getByRole('button',{name:'Save as project file',exact:true}).click();await expect(page.getByRole('button',{name:'Replace existing file'})).toBeVisible();await page.screenshot({path:'test-results/save-as.png'});await page.getByRole('button',{name:'Replace existing file'}).click();await expect(page.getByRole('status')).toContainText('Saved A.json');}
