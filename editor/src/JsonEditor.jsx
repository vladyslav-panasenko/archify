...
export default function JsonEditor({text,onChange,selectedPath,onFocus,children}) {
  ...
  return <div className="json-panel"><p>Edit source, then apply it. Suggestions modify unapplied text.</p>
    ...
    <label className="field">Schema suggestions at {path||'/'}<select value={choice} onChange={e=>setChoice(e.target.value)}><option value="">Choose a field or value</option>{options.map((o,i)=><option value={i} key={`${path}:${o.label}`}>{o.label}{o.required?' (required)':''}</option>)}</select></label>
    <button disabled={choice===''||!options[Number(choice)]} onClick={()=>{onChange(insertSuggestion(text,index,path,options[Number(choice)]));setChoice('');}}>Insert suggestion</button>
    <textarea ref={input} aria-label="Diagram JSON" spellCheck={false} value={text} onChange={e=>onChange(e.target.value)} onSelect={e=>{setCaret(e.target.selectionStart);setChoice('');}}/>
