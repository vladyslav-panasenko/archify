...
export function describeEdit(before, after) {
  ...
  if (
    fields.every((f) =>
      ["pos", "row", "col", "stage", "yOffset", "y"].includes(f),
    )
  )
    return `Move ${count} item${count === 1 ? "" : "s"}`;
  if (fields.every((f) => ["size", "width", "height"].includes(f)))
    return `Resize ${count} item${count === 1 ? "" : "s"}`;
  if (
    fields.every((f) =>
      [
        "via",
    ...
    return `Edit ${count} connection route${count === 1 ? "" : "s"}`;
  if (changes.every((c) => c.path.length === 2 && c.before === undefined))
    return `Add ${count} item${count === 1 ? "" : "s"}`;
  if (changes.every((c) => c.path.length === 2 && c.after === undefined))
    return `Remove ${count} item${count === 1 ? "" : "s"}`;
  if (fields.every((f) => ["label", "title", "description"].includes(f)))
    return `Edit ${count} label${count === 1 ? "" : "s"}`;
  if (
    changes.every((c) =>
      ["meta", "theme", "viewBox", "layout"].includes(c.path[0]),
    )
  )
    return "Change document settings";
...
export function historyEntries(state) {
  ...
  return snapshots.map((doc, index) => ({
    index,
    label: index
      ? describeEdit(snapshots[index - 1], doc)
...
export function jumpHistory(state, index) {
  if (
    ...
    index < 0 ||
    index > state.past.length + state.future.length
  )
    throw new Error("History position is unavailable.");
  let next = state;
