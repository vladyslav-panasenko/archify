...
export const authoringTypes=['architecture','workflow','dataflow'];
...
export function saveEdge(document,index,fields) {
 ...
 if(document.diagram_type==='dataflow'&&!edge.label?.trim())throw new Error('A flow label is required.');
...
export function deleteNode(document,id) {
 const next=clone(document),minimum=document.diagram_type==='dataflow'?2:1;if(sourceNodes(next).length<=minimum)throw new Error(`Keep at least ${minimum} nodes.`);
