  ...
  function read(path, depth = 0) {
    ...
    if (ch === "{") {
      ...
      while (text[offset] !== "}") {
        ...
        space();
        offset++;
        read(`${path}/${escapeKey(key)}`, depth + 1);
        space();
        if (text[offset] !== ",") break;
        offset++;
        space();
...
export function inspectJson(text) {
  ...
  try {
    ...
    const offset = Number(e.message.match(/position (\d+)/)?.[1] || 0);
    return {
      issues: [
        {
          path: "",
  ...
  const issues = valid
    ...
    : (
        validate?.errors || [
          {
            instancePath: "/diagram_type",
            message: "Choose a supported diagram type.",
          },
        ]
      ).map((e) => {
        let path = e.instancePath || "";
        if (e.params?.additionalProperty)
          path += `/${escapeKey(e.params.additionalProperty)}`;
...
export function schemaAt(type, path) {
  const root = schemaFor(type);
  let spec = root;
  for (const key of path.split("/").slice(1)) {
    spec = resolveSchema(spec, root);
    spec =
      spec?.properties?.[key.replace(/~1/g, "/").replace(/~0/g, "~")] ||
      spec?.prefixItems?.[Number(key)] ||
      spec?.items ||
      {};
  }
...
export function suggestions(index, path) {
  ...
  const range = index.ranges.get(path);
  if (!range) return [];
  const value = path
    .split("/")
    .slice(1)
    .reduce(
      (v, k) => v?.[k.replace(/~1/g, "/").replace(/~0/g, "~")],
      index.value,
    );
  if (spec.enum)
    return spec.enum.map((value) => ({
  ...
  if (spec.const !== undefined)
    return [
      { label: JSON.stringify(spec.const), value: spec.const, kind: "value" },
    ];
...
export function insertSuggestion(text, index, path, suggestion) {
  const range = index.ranges.get(path);
  if (!range) throw new Error("Select a valid JSON value first.");
  ...
  const end = range.end - 1,
    inner = text.slice(range.start + 1, end).trim();
  return (
    text.slice(0, end).trimEnd() +
    (inner ? "," : "") +
    "\n  " +
    JSON.stringify(suggestion.key) +
    ": " +
    JSON.stringify(suggestion.value) +
    "\n" +
    text.slice(end)
