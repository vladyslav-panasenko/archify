...
export function saveView(document,index,values){const next=clone(document);next.meta.views||=[];if(index===null&&next.meta.views.length>=5)throw new Error('A diagram supports up to five guided views.');if(!values.label.trim()||values.label.length>48||values.note?.length>140)throw new Error('Use a label of 1–48 characters and a note of at most 140 characters.');const focus=[...new Set(values.focus)];if(!focus.length||focus.some(id=>!next.components.some(c=>c.id===id)))throw new Error('Choose at least one existing component.');const value={...(index===null?{id:freshId(next.meta.views,'view')}:next.meta.views[index]),...values,focus};if(index===null)next.meta.views.push(value);else next.meta.views[index]=value;return next;}
export function deleteView(document,index){const next=clone(document);next.meta.views.splice(index,1);if(!next.meta.views.length)delete next.meta.views;return next;}
