...
export function resetFields(document,ids,kind,index=null) {
 const next=clone(document), removed=[];
 const clear=(item,keys,label)=>{for(const key of keys)if(Object.hasOwn(item,key)){delete item[key];removed.push(`${label}.${key}`);}};
 if(index!==null) {
  if(document.diagram_type==='sequence')throw new Error('Sequence message Y is required and cannot be reset.');
  const edge=connections(next)[index];if(!edge)throw new Error('Select a connection.');
  clear(edge,kind==='label'?['labelAt','labelDx','labelDy']:kind==='route'?['via']:[],edge.id||`connection ${index+1}`);
 }else{
  for(const item of sourceNodes(next).filter(c=>ids.includes(c.id))){
   if(document.diagram_type==='architecture') {
    if(kind==='position'&&next.layout&&Number.isInteger(item.row)&&Number.isInteger(item.col))clear(item,['pos'],item.id);
    if(kind==='size')clear(item,['size'],item.id);
   }else if(document.diagram_type!=='sequence')clear(item,kind==='position'?['yOffset']:kind==='size'?['width','height']:[],item.id);
  }
 }
