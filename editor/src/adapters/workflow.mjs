export const workflow = {
  ...
  fields: ["col", "yOffset"],
  regions(document) {
    return document.lanes.map((lane, index) => ({
      id: `lane:${index}`,
      label: lane.label,
      pos: [50, 90 + index * 180],
      size: [1220, 160],
  ...
  project(document, node) {
    ...
    return {
      ...node,
      pos: [80 + node.col * 200, 120 + lane * 180 + (node.yOffset || 0)],
      size: [node.width || 140, node.height || 64],
    };
  },
  move(document, node, [x, y]) {
    ...
    node.yOffset =
      Math.round(
        (y - 120 - document.lanes.findIndex((l) => l.id === node.lane) * 180) *
          100,
      ) / 100;
  },
  validate(document) {
