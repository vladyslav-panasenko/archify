...
export default function AutoLayoutPanel({document, selection, locked, onApply}) {
  const [preview,setPreview]=useState(null),[error,setError]=useState('');
  const boxes=preview?components(preview):[];
  const width=Math.max(400,...boxes.map(c=>c.pos[0]+c.size[0]+20)),height=Math.max(250,...boxes.map(c=>c.pos[1]+c.size[1]+20));
  return <div className="properties"><h2>Auto-arrange selection</h2>
    <p>Place unlocked selected architecture components in a stable grid, avoiding other components. Connections, waypoints and labels retain their authored values. The layout can extend beyond the canvas; inspect it before rendering.</p>
    {error&&<p role="alert">{error}</p>}
    {!preview?<button disabled={document.diagram_type!=='architecture'||!selection.length} onClick={()=>{try{setPreview(autoLayout(document,selection,locked));setError('');}catch(e){setError(e.message);}}}>Preview arrangement</button>:<>
