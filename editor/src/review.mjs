...
export function documentChanges(before, after) {
  ...
  function walk(a, b, path) {
    ...
    if (
      ...
      const keyed =
        [...a, ...b].every((v) => v && typeof v.id === "string") &&
        new Set(a.map((v) => v.id)).size === a.length &&
        new Set(b.map((v) => v.id)).size === b.length;
      if (keyed) {
        for (const id of new Set([...a, ...b].map((v) => v.id)))
          walk(
            a.find((v) => v.id === id),
            b.find((v) => v.id === id),
            [...path, id],
    ...
    const category =
      a === undefined || b === undefined
        ? path.length <= 2
          ? "topology"
          : layoutFields.has(path.at(-1))
            ? "layout"
            : "content"
        : layoutFields.has(path.at(-1)) || path[0] === "layout"
          ? "layout"
          : ["from", "to", "id", "lane", "focus", "wraps"].includes(path.at(-1))
            ? "topology"
            : "content";
...
export function mergeDocuments(base, local, remote, choices = {}) {
  ...
  function conflict(b, l, r, path) {
    ...
    if (!["local", "remote"].includes(choices[key]))
  ...
  function merge(b, l, r, path) {
    ...
    if (
      [b, l, r].every(Array.isArray) &&
      [b, l, r].every(
        (a) =>
          a.every((v) => object(v) && typeof v.id === "string") &&
          new Set(a.map((v) => v.id)).size === a.length,
      )
    ) {
      const values = new Map();
      for (const id of new Set([...b, ...l, ...r].map((v) => v.id))) {
        const value = merge(
          b.find((v) => v.id === id),
          l.find((v) => v.id === id),
          r.find((v) => v.id === id),
      ...
      return [...new Set([...order, ...values.keys()])]
  ...
  return {
    document: structuredClone(merge(base, local, remote, [])),
    conflicts,
  };
}
