...
export function assertResourceLimits(value) {
  const stack = [[value, 0, false]], ancestors = new Set();
  let count = 0;
  while (stack.length) {
    const [item, depth, leaving] = stack.pop();
    if (leaving) { ancestors.delete(item); continue; }
    ...
    if (item && typeof item === "object") {
      if (ancestors.has(item)) throw new Error("Document must be an acyclic JSON tree.");
      ancestors.add(item);
      stack.push([item, depth, true]);
      for (const child of Object.values(item)) stack.push([child, depth + 1, false]);
