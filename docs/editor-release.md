...
From `editor/`, run `npm ci`, `npm run check:release:browser`, and `npm run package:desktop`. The release check regenerates the schema inventory, runs Node tests, builds production assets, measures the deterministic benchmark, and optionally runs all browser tests. Packaging creates `release/archify-local-editor-<version>.tgz` containing the production UI, browser-only offline build, desktop app-window launcher, local server, five compiler renderers and schemas, examples, dependency manifest, license, and notices. It also writes a SHA-256 file and desktop release manifest. Extract it outside the development checkout, run `npm ci`, start it in server and desktop modes, and complete open/edit/save/render/stop before recording package acceptance.

The 0.2.0 package is unsigned. A release owner must sign the immutable archive or platform installer with an approved identity, publish the matching checksum and detached signature, and exercise install, update, and rollback on every claimed platform. `editor/desktop/trusted-update.mjs` verifies the archive checksum and an Ed25519 signature over that checksum; it does not download or install anything. Browser-only offline behavior, desktop lifecycle, local review/history, interchange, extensions, packs, and architecture v2 ports are documented in [Local product expansions](editor-local-expansions.md).
...
Each editor release uses the compiler and schemas shipped in the same package. Mixing files from another release is unsupported. Diagram versions stay unchanged during open, canvas editing, JSON editing, render and save. A supported migration must show original and proposed JSON, retain a downloadable original, validate the candidate, and apply only after the user selects Apply. Workflow v1→v2 and architecture v1→v2 are supported explicit migrations; architecture migration adds no inferred ports.
...
The [0.1.0 acceptance record](editor-release-acceptance-2026-09-14.md) is historical evidence for that exact candidate. Editor 0.2.0 changes code, schemas, packaging, and supported workflows, so named owners must record a new candidate hash and repeat the applicable acceptance before describing it as signed off.
