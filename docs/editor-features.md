...
The editor edits Archify JSON. Archify owns final HTML rendering. The first batch is complete; the next batch below is proposed. No feature requires visual parity with Archify's HTML.
...
F11–F28 are approved for implementation. **Implemented: F11–F20.** Remaining features are pending. Priority reflects the benefit to manual refinement: P1 removes common editing friction, P2 expands visual authoring, and P3 improves larger-document workflows.
